import { SimpleMessage, RelatedDocument } from '../../../@types/conversation';
...
export const convertThinkingLogToAgentToolProps = (
  ...
  return thinkingLog.flatMap((message) => {
    ...
    const tools: Record<string, AgentToolUse> = Object.fromEntries(
      message.content.flatMap((content) => {
...
export const getRelatedDocumentsOfToolUse = (
  relatedDocuments: RelatedDocument[] | undefined,
  toolUseId: string
) => {
  ...
  const filteredWithToolUseId = relatedDocuments.filter(
    (document) => document.sourceId === toolUseId
  );
  if (filteredWithToolUseId.length > 0) {
    return filteredWithToolUseId;
  }

  const relatedDocumentsSortedByRank = relatedDocuments
    .flatMap((document) => {
      const match = document.sourceId.match(
        /(?<sourceId>.+?)@(?<rank>[\d]+?)$/
      );
